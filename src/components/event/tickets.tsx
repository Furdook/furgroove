export default function Tickets() {
  ...
  return (
    <section>
      <h2 className="text-accent-pink pb-4 text-2xl uppercase">Tickets</h2>
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        <div className="flex flex-col gap-4">
          <div className="my-auto flex flex-col gap-1">
            <h3 className="text-xl">Your ticket to FurGroove</h3>
            <h4 className="text-lg opacity-75">Tickets for this event come in three tiers</h4>
          </div>
          <a
            target="_blank"
        ...
        <div className="flex gap-2 sm:gap-4">{tickets.map((ticket) => renderTicket(ticket))}</div>
        <a
          target="_blank"
          href="https://eventix.shop/83fg7sdc"
          rel="noreferrer"
